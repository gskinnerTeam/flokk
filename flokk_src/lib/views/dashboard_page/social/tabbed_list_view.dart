...
class TabbedListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        /// Tab Bg
        Container(decoration: buildTabDec(true), foregroundDecoration: buildTabDec(false)),
  ...
  Path getOuterPath(Rect rect, {TextDirection? textDirection}) {
    ...
    void drawBody(Path p) {
      ...
      p.addRRect(RRect.fromRectAndCorners(tabRect,
          topLeft: Radius.zero, bottomLeft: radius, topRight: Radius.zero, bottomRight: radius));
    }

    void drawTab(Path p, bool rightSide) {
      double xPos = rightSide ? rect.width * .5 : 0;
      Rect tabRect = Rect.fromLTWH(rect.left + xPos, rect.top, rect.width * .5, barHeight);
      p.addRRect(RRect.fromRectAndCorners(tabRect,
          topLeft: radius, bottomLeft: Radius.zero, topRight: radius, bottomRight: Radius.zero));
    }
