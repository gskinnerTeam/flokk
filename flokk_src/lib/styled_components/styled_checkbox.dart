...
class StyledCheckbox extends StatelessWidget {
  ...
  const StyledCheckbox({Key? key, this.value = StyledCheckboxValue.False, this.size = 18, this.onChanged})
      : super(key: key);

  void _handleTapUp(TapUpDetails details) {
    switch (value) {
      case StyledCheckboxValue.True:
        onChanged?.call(StyledCheckboxValue.False);
        break;
      case StyledCheckboxValue.False:
        onChanged?.call(StyledCheckboxValue.Partial);
        break;
      case StyledCheckboxValue.Partial:
        onChanged?.call(StyledCheckboxValue.True);
        break;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          color: value == StyledCheckboxValue.False ? Colors.transparent : theme.accent1Darker,
          borderRadius: Corners.s3Border,
          border: Border.all(color: value == StyledCheckboxValue.False ? theme.grey : theme.accent1Darker, width: 1.5)),
